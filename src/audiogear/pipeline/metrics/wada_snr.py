...
class SnrMetric(BaseMetric):
    name = "💥 Snr"
    parallel_cpu = True
    _requires_dependencies = ("librosa", "tqdm", "numpy")
    def __init__(self,
        num_threads: int = -1, file_writer: DiskWriter = None, file_reader: BaseDiskReader = None, workers: int = 1):
        super().__init__(metric="wada_snr", file_writer=file_writer, file_reader=file_reader, num_threads=num_threads)
    ...
    def compute_metric(self, segment: AudioSegment) -> int | float | bool:
        ...
        # init
        wav = segment.audio_file
        wav, _ = librosa.load(wav, sr=segment.sample_rate)
        eps = 1e-10
