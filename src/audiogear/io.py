...
class DataFolder(DirFileSystem):
    ...
    def list_files(
        ...
        if glob_pattern and not has_magic(glob_pattern):
            # A bare extension (".csv") is a convenience for "*.csv". A full file
            # name ("metadata.csv") is matched EXACTLY — otherwise "*metadata.csv"
            # would also pick up "extended_metadata.csv" and similar.
        ...
        return sorted(
            [
                f
                for f, info in (
                    self.find(subdirectory, maxdepth=1 if not recursive else None, detail=True)
                    if not glob_pattern
                    else self.glob(
    ...
    def resolve_path(self, path: str) -> str:
        if isinstance(self.fs, LocalFileSystem):
            # For local file system, strip file:// and similar protocols
            return self.fs._strip_protocol(self._join(path))
        else:
            # For non-local file systems, explicitly add back the protocol
            return self.fs.unstrip_protocol(self._join(path))

    def is_file_empty(self, file_path):
        return True if self.fs.info(file_path)['size'] == 0 else False
